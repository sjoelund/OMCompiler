...
//  print(anyString(simfunc) + "\n");
  ...
  case SimCodeFunction.FUNCTION(name, outVars, functionArguments, variableDeclarations, body, visibility, info)
    ...
    for simcodeVar in outVars loop
      ...
      DoubleEndedList.push_back(outputs,tmpVar);
    end for;
    for simcodeVar in variableDeclarations loop
      tmpVar := ConvertSimCodeVars(simcodeVar, state,true);
      DoubleEndedList.push_back(state.locals, tmpVar);
    end for;
  ...
  algorithm
    ...
    midOutVars := DoubleEndedList.toListAndClear(outputs);
      ...
      case DAE.STMT_ASSIGN(ty, exp1 as DAE.CREF(__), exp, _)
        algorithm
         ...
         () := match cref
            ...
            case DAE.CREF_IDENT(_ ,DAE.T_ARRAY(__),subscriptLst=subscriptLst)
            algorithm
...
//  print("\n Calling ExpToMid" + anyString(exp) + "\n\n\n");
  ...
  local
    ...
    list<DAE.Subscript> subscripts;
    MidCode.RValue rvalue;
    DoubleEndedList<MidCode.RValue> rValues;
...
algorithm
  ...
  algorithm
    ...
    stateTerminate(labelNext,
                    MidCode.CALL(path,callattr.builtin,DoubleEndedList.toListAndClear(inputs),outvars,labelNext,callattr.ty),
                    state);
  then ();
  end match;
end CallToMid;
...
algorithm
  block_ := MidCode.BLOCK(stateGetCurrentLabel(state),
                          ...
                          terminator);
  DoubleEndedList.push_back(state.blocks, block_); //Add the block to the list of blocks.
...
//        print("The patterns:"+anyString(patterns) + "\n");
          ...
          then /*I think and I am pretty sure the patterns have to be reversed first because of the recursion - John*/
            patternToMidCode(state=state, matches=List.threadTuple(inputsMidVar,listReverse(patterns)), labelNoMatch=labelMux);
          else
          patternToMidCode(state=state, matches=List.threadTuple(inputsMidVar,listReverse(patterns))
