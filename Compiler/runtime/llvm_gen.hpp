...
extern "C"
{
  ...
  */
  ...
  struct Program {
    ...
    //Can be extended to configure the different optimisations on setup.
    Program(const std::string &name) :  builder{context}
    {
      ...
      /*Interprocedual, only do inlining for now to keep the JIT snappy TODO:Move to JIT Module?*/
      ...
      jit = llvm::make_unique<llvm::orc::OMCJIT>();
    }
    ...
    /*Runs interprocedual optimisations*/
    void runIPO() {
      ipoPassMngr.run(*program->module);  //TODO. Move this to another location?
    }
