...
/*Used for construction of binary instructions among other things to reduce bloat. */
...
void dumpIR()
{
  //OBS!, cannot link llvm 5 to dump see https://stackoverflow.com/questions/46367910/llvm-5-0-linking-error-with-llvmmoduledump
  if (!program->module) {
...
int jitCompile()
{
  ...
  if (!targetAdress) {
    fprintf(stderr,"Failed JIT compilation, no top level expression in module\n");
    return 1;
  }

  /*Get the address of the compiled to level expression. Assign said address to FP.*/
  adress = targetAdress.get();
  program->top_level_func = reinterpret_cast<modelica_metatype(*)(modelica_metatype)>(adress);
...
{
  DBG("Calling initGen\n");
  /*Sets some global variables in LLVM, important program crashes without these lines...*/
  ...
  /*
    ...
    If there it is possible to do this and include all these functions we can have inlined calls
  ...
  //    program->module  = llvm::parseIRFile("SOME_FILE_PATH_TO_PRECOMPILED_BITCODE/llvm_gen_wrappers.bc"
  //                                         ,Err
  //                                         ,program->context);
  //    if(!program->module) {
...
void setOptSettings(const modelica_boolean b)
{
  program->shallOptimize = b;
}
...
{
  /*Set the function we are currently generating, std::make_unique is introduced in C++14 */
  std::shared_ptr<Function> func = program->functions[name];

  if (!func) {
    func =  std::shared_ptr<Function>(new Function(llvm::make_unique<FunctionPrototype>()));
...
int createExit(const modelica_integer exitId)
{
  ...
  /*Block have to be created here aswell since midcode can specify branches to this point*/
...
int createBranch(const char *conditionVar, const modelica_integer onTrue, const modelica_integer onFalse)
{
...
{
  return program->currentFunc->imValMngr->createSwitch(createLoadInst(condVar)
                                                       ,numCases,program->builder);
}
...
int addCaseToSwitch(const modelica_integer onVal, const modelica_integer dest)
{
  ...
  auto constantInt {llvm::ConstantInt::get(llvm::Type::getIntNTy(program->context,BITS)
                                           ,onVal
                                           ,true)};
...
/* creates the arguments for the function prototype that is currently being processed */
int createFunctionProtArg(const uint8_t type, const char *name)
{
  /* Adds the argument to the prototypes argument vector, then name said argument.
     When nameArgument is called it is placed in a vector holding the names of the function
     parameters.
   */
  llvm::Type *ty = {getLLVMType(type)};

  if (!ty) {
    fprintf(stderr,"Tried to generate a function argument with an invalid type\n");
    return 1;
  }
...
/* Creates the function type for the function prototype for the function that is currently generated */
int createFunctionType(uint8_t type, const char *structName /*To fetch the correct struct*/)
{
  program->currentFunc->setPrototypeFunctionType(llvm::FunctionType::get(getLLVMType(type,structName)
                                                                         ,program->currentFunc->getPrototypeArgs()
                                                                         ,false));
  return 0;
}
...
int createFunctionPrototype(const char *name)
{
  ...
  program->currentFunc->setLLVMFunction(llvm::Function::Create(program->currentFunc->getPrototypeFunctionType()
...
/* Call starts by an external call from genEntry this function first
  fetch the function by name, given by genEntry and proceeds by generating the first basic block,
...
/*
   Fetch mmc_jumper from threadData struct (I do not know the signature of the threadData struct
   in the llvm context.)
*/
...
int createCallArgMmcJmpr()
{
  ...
  auto ft = llvm::FunctionType::get(getLLVMType(MODELICA_METATYPE),std::vector<llvm::Type*>(), false);
...
{
  llvm::Value *v{llvm::ConstantInt::get(llvm::Type::getIntNTy(program->context,BITS),arg,true)};
  program->currentFunc->callArgs.push_back(v);
  return 0;
}

/* Creates a call instruction, note that the argument that is passed to this external call should
               ...
               , modelica_boolean assignment, modelica_boolean isVariadic)
{
  ...
  llvm::Function *f {program->module->getFunction(name)};
  /* f == nullptr is also the case when we are dealing with an external call to a compiled function present somewhere in the compiler.
     This call should be of two types, a void function used for side effects, or a function returning a specific value.
     For the first case we will generate a declaration for the function with the correct parameters so that the Execution
     engine of the JIT can locate it at a later stage. For the second option we will generate a function prototype of
  ...
  */
  ...
  if (assignment) {
    llvm::Value *s = program->builder.CreateCall(f,program->currentFunc->callArgs,"calltmp");
...
int createLongJmp()
{
  ...
  if (!f) {
    std::vector<llvm::Type*> args {getLLVMType(MODELICA_METATYPE),getLLVMType(MODELICA_INTEGER)};
    llvm::FunctionType *ft{llvm::FunctionType::get(llvm::Type::getVoidTy(program->context)
                                                  ,args,false)};
    f = llvm::Function::Create(ft,llvm::Function::ExternalLinkage,"longjmp",program->module.get());
    /*Set attributes so that LLVM does not optimise away the function during opt phase.*/
    llvm::Attribute attr { llvm::Attribute::get(program->context, "noinline", "true") };
...
/*Allocates a llvm::Type integer on the stack with nBits in size. Observe that
 the size of integer in the top level expression is 64 bits but when it is unboxed it is 63 = (62) + signed bit. A 63 bit representation was tried, but it led to difficulties in codegen since
...
{
  llvm::Function *f {createExternalCallDecl("get_mmc_jumper",MODELICA_METATYPE,{})};
...
int storeLiteralInt(const int64_t src, const char *dest, const uint8_t nBits)
{
  llvm::Value *s {llvm::ConstantInt::get(llvm::Type::getIntNTy(program->context,nBits),src,true)};
  createStoreInst(s,dest);
  return 0;
}
...
int storeLiteralReal(const double src,const char *dest)
{
  ...
  llvm::Value *s {llvm::ConstantFP::get(llvm::Type::getDoubleTy(program->context),src)};
  createStoreInst(s,dest);
  return 0;
}
...
int storeLiteralBoolean(const modelica_boolean src, const char *dest)
{
  ...
  llvm::Value *s {llvm::ConstantInt::get(llvm::Type::getInt1Ty(program->context),src)};
  createStoreInst(s,dest);
  return 0;
}

/* Stores the address of a 64 bit pointer variable.
(The pointer variables are passed arround as i8 for compatability with C but they are infact 8 bytes big)*/
...
{
  DBG("Calling storeThreadData_t with address of threadData:%p and dest name:%s\n",threadData,threadDataID);
...
int createReturnZero()
{
  ...
  llvm::Value *retVal{llvm::ConstantInt::get(llvm::Type::getInt64Ty(program->context),0,true)};
...
int createIntToDouble(const char *src, const char *dest)
{
  ...
  DBG("Calling createIntToDouble with %s %s\n",src,dest);
  //Passing the appropriate builder function call, this is done to avoid dublication.
  auto f { std::bind(&llvm::IRBuilder<>::CreateSIToFP,&program->builder,_1,_2,"intDoubleTmp")};
  return castXTypeToYType(src, dest, llvm::Type::getDoubleTy(program->context),f);
}

int createDoubleToInt(const char *src, const char *dest)
{
  DBG("Calling createDoubleToInt with %s %s\n",src,dest);
  auto f { std::bind(&llvm::IRBuilder<>::CreateFPToSI,&program->builder,_1,_2,"doubleToIntTmp")};
  return castXTypeToYType(src, dest, llvm::Type::getInt64Ty(program->context),f);;
}

//For some stupid reason bind does not work with CreateIntCast, ugly but will not waste more time with it, see commented line.
...
int createIntToMeta(const char *src, const char *dest)
{
  ...
  auto f {std::bind(&llvm::IRBuilder<>::CreateIntToPtr,&program->builder,_1,_2,"intToMetaTmp")};
  return castXTypeToYType(src,dest,llvm::Type::getInt8PtrTy(program->context),f);;
}

int createMetaToInt(const char *src, const char *dest)
{
  DBG("createMetaToInt %s %s",src,dest);
  auto f {std::bind(&llvm::IRBuilder<>::CreatePtrToInt,&program->builder,_1,_2,"intToMetaTmp")};
  return castXTypeToYType(src,dest,llvm::Type::getInt64Ty(program->context),f);;
}
...
int createDoubleToBool(const char *src, const char *dest)
{
  ...
  auto f { std::bind(&llvm::IRBuilder<>::CreateFPToSI,&program->builder,_1,_2,"doubleToBoolTmp")};
  return castXTypeToYType(src, dest,llvm::Type::getInt1Ty(program->context),f);;
}

int createBoolToDouble(const char *src, const char *dest)
{
  DBG("createBoolToDouble %s %s\n",src,dest);
  auto f { std::bind(&llvm::IRBuilder<>::CreateFPToSI,&program->builder,_1,_2,"boolToDouble")};
  return castXTypeToYType(src, dest,llvm::Type::getDoubleTy(program->context),f);;
}
//TODO, this function will be called if the situation arises (MidToLLVM.mo will call it if it would occur)
int createMetaToDouble(const char *src, const char *dest)
{
  DBG("CONVERTING A METATYPE TO DOUBLE DOES NOT SEEM TO BE SUPPORTED,INVESTIGATE\n");
  return 0;
}
//TODO Same as above.
int createDoubleToMeta(const char *src,  const char *dest)
{
  DBG("CONVERTING A DOUBLE TO META DOES NOT SEEM TO BE SUPPORTED,INVESTIGATE\n");
  return 0;
}
...
int createPow(const char *lhs, const char *rhs, const char *dest)
{
  ...
  if ( !(f = program->module->getFunction("pow"))) {
      llvm::FunctionType *ft {llvm::FunctionType::get(llvm::Type::getDoubleTy(program->context),
                                                      {l->getType(),r->getType()}, false)};
      f = llvm::Function::Create(ft,llvm::Function::ExternalLinkage,
                                 "pow", program->module.get());
    }
...
/*Considering a modelica integer is 32 bits on windows some macros might be needed for the functions below*/
int createIAdd(const char *lhs, const char *rhs,const char *dest)
{
  DBG("Creating IADD\n");
  auto f {std::bind(&llvm::IRBuilder<>::CreateAdd,&program->builder,_1,_2,_3,false,false)};
...
{
  DBG("Creating ISUB\n");
  auto f {std::bind(&llvm::IRBuilder<>::CreateSub,&program->builder,_1,_2,_3,false,false)};
...
{
  DBG("Creating IMUL\n");
  auto f {std::bind(&llvm::IRBuilder<>::CreateMul,&program->builder,_1,_2,_3,false,false)};
...
{
  DBG("Creating IDiv");
  auto f { std::bind(&llvm::IRBuilder<>::CreateSDiv,&program->builder,_1,_2,_3,false) };
  return createBinop(lhs,rhs,dest,"divI64Rmp",f);
}
//TODO refactor these below later with some tweaking we can also use createBinop for them aswell.
int createILess(const char *lhs, const char *rhs,const char *dest)
{
  DBG("Creating ILess\n");
  llvm::Value *l,*r,*d;
  binopInit(lhs,rhs,dest,l,r,d);
  llvm::Value *res{program->builder.CreateICmp(llvm::ICmpInst::ICMP_SLT,l,r,"lessItmp")};
...
int createILessEq(const char *lhs, const char *rhs, const char *dest)
{
  ...
  llvm::Value *l,*r,*d;
  binopInit(lhs,rhs,dest,l,r,d);
  llvm::Value *res{program->builder.CreateICmp(llvm::ICmpInst::ICMP_SLE,l,r,"lessEqItmp")};
  createStoreInst(res,dest);
  return 0;
}
...
int createIGreater(const char *lhs,const char *rhs,const char *dest)
{
  ...
  llvm::Value *l,*r,*d;
  binopInit(lhs,rhs,dest,l,r,d);
  llvm::Value *res{program->builder.CreateICmp(llvm::ICmpInst::ICMP_SGT,l,r,"greaterItmp")};
  createStoreInst(res,dest);
  return 0;
}
...
int createIGreaterEq(const char *lhs,const char *rhs,const char *dest)
{
  ...
  llvm::Value *l,*r,*d;
  binopInit(lhs,rhs,dest,l,r,d);
  llvm::Value *res{program->builder.CreateICmp(llvm::ICmpInst::ICMP_SGE,l,r,"greaterItmp")};
  createStoreInst(res,dest);
  return 0;
}
...
int createIEqual(const char *lhs, const char *rhs,const char *dest)
{
  ...
  llvm::Value *l,*r,*d;
  binopInit(lhs,rhs,dest,l,r,d);
  llvm::Value *res{program->builder.CreateICmp(llvm::ICmpInst::ICMP_EQ,l,r,"eqItmp")};
  createStoreInst(res,dest);
  return 0;
}
...
int createINequal(const char *lhs, const char *rhs,const char *dest)
{
  ...
  llvm::Value *l,*r,*d;
  binopInit(lhs,rhs,dest,l,r,d);
  llvm::Value *res{program->builder.CreateICmp(llvm::ICmpInst::ICMP_NE,l,r,"neqItmp")};
  createStoreInst(res,dest);
  return 0;
}
...
int createDLess(const char *lhs, const char *rhs, const char *dest)
{
  ...
  llvm::Value *l,*r,*d;
  binopInit(lhs,rhs,dest,l,r,d);
  llvm::Value *res{program->builder.CreateFCmp(llvm::FCmpInst::FCMP_ULT,l,r,"lessDtmp")};
  createStoreInst(res,dest);
  return 0;
}
...
int createDLessEq(const char *lhs, const char *rhs, const char *dest)
{
  ...
  llvm::Value *l,*r,*d;
  binopInit(lhs,rhs,dest,l,r,d);
  llvm::Value *res{program->builder.CreateFCmp(llvm::ICmpInst::FCMP_ULE,l,r,"lessEqDtmp")};
  createStoreInst(res,dest);
  return 0;
}
...
int createDEqual(const char *lhs, const char *rhs, const char *dest)
{
  ...
  llvm::Value *l,*r,*d;
  binopInit(lhs,rhs,dest,l,r,d);
  llvm::Value *res{program->builder.CreateFCmp(llvm::ICmpInst::FCMP_UEQ,l,r,"eqDtmp")};
  createStoreInst(res,dest);
  return 0;
}
...
int createDNequal(const char *lhs, const char *rhs, const char *dest)
{
  ...
  llvm::Value *l,*r,*d;
  binopInit(lhs,rhs,dest,l,r,d);
  llvm::Value *res{program->builder.CreateFCmp(llvm::ICmpInst::FCMP_UNE,l,r,"neqDtmp")};
  createStoreInst(res,dest);
  return 0;
}
...
int createDGreater(const char *lhs,const char *rhs,const char *dest)
{
  ...
  llvm::Value *l,*r,*d;
  binopInit(lhs,rhs,dest,l,r,d);
  llvm::Value *res{program->builder.CreateFCmp(llvm::ICmpInst::FCMP_UGT,l,r,"greaterI64tmp")};
  createStoreInst(res,dest);
  return 0;
}
...
int createDGreaterEq(const char *lhs,const char *rhs,const char *dest)
{
  ...
  llvm::Value *l,*r,*d;
  binopInit(lhs,rhs,dest,l,r,d);
  llvm::Value *res{program->builder.CreateFCmp(llvm::ICmpInst::FCMP_UGE,l,r,"greaterEQI64tmp")};
  createStoreInst(res,dest);
  return 0;
}
/* For modelica_booleans, observe since llvm uses 2's-complement and a modelica_boolean is represented in llvm
...
int createBGreater(const char *lhs,const char *rhs,const char *dest)
{
  ...
  llvm::Value *l,*r,*d;
  binopInit(lhs,rhs,dest,l,r,d);
  llvm::Value *res{program->builder.CreateICmp(llvm::ICmpInst::ICMP_UGT,l,r,"greaterI8tmp")};
  createStoreInst(res,dest);
  return 0;
}
...
int createBGreaterEq(const char *lhs,const char *rhs,const char *dest)
{
  ...
  llvm::Value *l,*r,*d;
  binopInit(lhs,rhs,dest,l,r,d);
  llvm::Value *res{program->builder.CreateICmp(llvm::ICmpInst::ICMP_UGE,l,r,"greaterEQI8tmp")};
  createStoreInst(res,dest);
  return 0;
}
...
int createBLess(const char *lhs, const char*rhs ,const char *dest)
{
  ...
  llvm::Value *l,*r,*d;
  binopInit(lhs,rhs,dest,l,r,d);
  llvm::Value *res{program->builder.CreateICmp(llvm::ICmpInst::ICMP_ULT,l,r,"lessI1tmp")};
  createStoreInst(res,dest);
  return 0;
}
...
int createBLessEq(const char *lhs, const char *rhs, const char *dest)
{
  ...
  llvm::Value *l,*r,*d;
  binopInit(lhs,rhs,dest,l,r,d);
  llvm::Value *res{program->builder.CreateICmp(llvm::ICmpInst::ICMP_ULE,l,r,"lessI1tmp")};
  createStoreInst(res,dest);
  return 0;
}
...
int createBEqual(const char *lhs, const char *rhs , const char *dest)
{
  ...
  llvm::Value *l,*r,*d;
  binopInit(lhs,rhs,dest,l,r,d);
  llvm::Value *res{program->builder.CreateICmp(llvm::ICmpInst::ICMP_EQ,l,r,"EqI1tmp")};
  createStoreInst(res,dest);
  return 0;
}
...
int createBNequal(const char *lhs, const char *rhs , const char *dest)
{
  ...
  llvm::Value *l,*r,*d;
  binopInit(lhs,rhs,dest,l,r,d);
  llvm::Value *res{program->builder.CreateICmp(llvm::ICmpInst::ICMP_NE,l,r,"EqI1tmp")};
  createStoreInst(res,dest);
  return 0;
}

//TODO, kapsulera in detta i en klass enklare att förstå då? blir formodligen samma elande men mer kod..
static llvm::Value *cdr = nullptr; //The point of this variable is to keep the state when generating lists.
...
//TODO refactor this...
int createMmcCons(const char *carElement /*This refeers to a mmc already created.*/)
{
  DBG("CreateMmcCons\n");
  /*Fetch the arguments*/
  ...
  if (!cdr) {
    ...
    if (!f) {
      ...
      f = llvm::Function::Create(ft,llvm::Function::ExternalLinkage
                                 ,"mmc_mk_cons_last_elem",program->module.get());
    }
    //Call it
...
/*Functions relating to LLVM structs. Used for generation of tuples and uniontypes (That are then sent to the OMC for creation
).*/
...
int createStructSignature(const char *sName)
{
  ...
  /*Get the sField. The destructor will be called automatically at the end of this function*/
  std::vector<llvm::Type*> sField = program->currentFunc->imValMngr->getStructField();
...
int createGlobalStructFieldConstant(const char *varName)
{
  ...
  if (!c) {
    fprintf(stderr,"Cannot invoke createGlobalStructFieldConstant with unallocated variable:%s\n",varName);
    dumpIR();
...
/*The indent of this function is to generate LLVM IR for structs created in the global context, with global variables.*/
int createGlobalStructConstant(const char *structTypeName,const char *name) {
  DBG("structTypeName:%s name:%s file:%s function:%s line%d\n",structTypeName,name, __FILE__,__FUNCTION__,__LINE__);
  ...
  for (const auto &llvmC : field) {
    llvm::Constant *idx0 {llvm::ConstantInt::get(llvm::Type::getInt32Ty(program->context),0)};
    /*TODO: Add if stmt here to check if we are dealing with an aggregate glb var or not (only works for aggregates)*/
    const std::vector<llvm::Constant*> indices{idx0,idx0};
    auto ty =  llvmC->getType()->getElementType(); /* We must get the encapsulated type*/
    llvm::Constant *gep = llvm::ConstantExpr::getGetElementPtr(ty,llvmC,indices);
    gepField.push_back(gep);
  }

  DBG("Creating gvArStruct\n");
  llvm::GlobalVariable *gvarStruct = new llvm::GlobalVariable(*program->module.get(),
                                                              program->module->getTypeByName(structTypeName),
                                                              false,
  ...
  llvm::Constant* constStruct = llvm::ConstantStruct::get(program->module->getTypeByName(structTypeName),gepField);
...
int createStoreDoubleToPtr(double src,const char *dest,const uint8_t idx0)
{
  ...
  llvm::Value *sV {llvm::ConstantFP::get(llvm::Type::getDoubleTy(program->context),src)};
  llvm::Value *dV {createLoadInst(dest)};
  llvm::Value *gep {program->builder.CreateInBoundsGEP(dV,vIdx0,"gepTmp")};
  //All these functions of similar type can look the same, just add the right bitcast in a later refactor.
...
int createStoreDVarToPtr(const char *src,const char *dest,const uint8_t idx0)
{
...
/*For N dimensional structs, store to the fields of the structs using idx0 and idx1 for indexing.*/
...
int createStoreFromStruct(const char *varName,const char *structName,const uint8_t idx0,const uint8_t idx1)
{
  ...
  llvm::LoadInst *structElemL = program->builder.CreateLoad(structElem,structElem->getName());
...
int createGVarCStr(const char *origName)
{
  ...
  llvm::GlobalVariable *gStr = new llvm::GlobalVariable(*program->module.get()
                                                        ,llvm::ArrayType::get(/*i8, regular chars*/llvm::Type::getInt8Ty(program->context),strlen(origName)+1)
                                                        ,true
                                                        ,llvm::GlobalValue::PrivateLinkage
                                                        ,0
                                                        );
  auto cs = llvm::ConstantDataArray::getString(program->context, origName, true);
  gStr->setAlignment(1);
  gStr->setInitializer(cs);
  /*TODO: Gvar to the global variables map (Observe that this might lead to scope problem if a variable would have the same name as a string with gbl_ prefix)*/
...
{
  DBG("function:%s file:%s line:%d name:%s fields:%ld\n",__FUNCTION__,__FILE__,__LINE__,name,fields);
  /*Fetch the strings that we are going to make an array from (the destructor will clear it when this func exits)*/
  std::vector<std::string> llvmStrArr { program->llvmStrArray  };
  DBG("llvmStrArr fetched\n");
  std::vector<llvm::Constant*> gepArray; //Holds GEP instructions (The char array should be an array of such insts)
  ...
  for (const auto &s : program->llvmStrArray) {
    ...
    llvm::GlobalVariable *gStr = new llvm::GlobalVariable(*program->module.get()
                                                          ,llvm::ArrayType::get(getLLVMType(MODELICA_METATYPE),s.length()+1) //+ 1 to include CString terminator.
                                                          ,true
                                                          ,llvm::GlobalValue::PrivateLinkage
                                                          ,0
                                                          );
    gStr->setAlignment(1); //These are C style strings (char*), not Modelica strings.
    llvm::Constant *strCData = llvm::ConstantDataArray::getString(program->context, s, true);
    gStr->setInitializer(strCData);     //Initialize the Gvar.
    llvm::Constant *idx0 {llvm::ConstantInt::get(llvm::Type::getInt32Ty(program->context),0)};
    llvm::Constant *idx1 {llvm::ConstantInt::get(llvm::Type::getInt32Ty(program->context),0)};
    std::vector<llvm::Constant*> indices {idx0,idx1};
    llvm::Constant *gep = llvm::ConstantExpr::getGetElementPtr(strCData->getType(),gStr,indices);
  ...
  llvm::GlobalVariable *gvarStrArray = new llvm::GlobalVariable(*program->module.get(),
  ...
  llvm::Constant *constArray = llvm::ConstantArray::get(llvm::ArrayType::get(getLLVMType(MODELICA_METATYPE),fields)
...
{
  DBG("function:%s file:%s line:%d variable:%s",__FUNCTION__,__FILE__,__LINE__,elem);
  program->llvmStrArray.push_back(elem);
  return 0;
}

/* Creates a Modelica_String, the GlobalStringPtr will be reduced to private linkage by LLVM-opt */
int createStringConstant(const char *str,char *dest)
{
  ...
  llvm::FunctionType *ft{llvm::FunctionType::get(getLLVMType(MODELICA_METATYPE),argTypes,false)};
  DBG("createStringConstant string:%s dest:%s file:%s line:%d\n",str,dest,__FILE__,__LINE__);

 if (!f) {
    f = llvm::Function::Create(ft, llvm::Function::ExternalLinkage, fName, program->module.get());
  }
