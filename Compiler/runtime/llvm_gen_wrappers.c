...
{
  double d =  mmc_prim_get_real(v);
  return d;
}

modelica_integer mmc_unbox_integer_no_inline(modelica_metatype v)
{
  modelica_integer i = MMC_UNTAGFIXNUM(v);
  return i;
}
...
modelica_metatype mmc_icon_to_value_wrapper(modelica_metatype mmc)
{
    if (MMC_IS_INTEGER(mmc)) {
	  modelica_metatype v = Values__INTEGER(mmc_mk_icon(MMC_UNTAGFIXNUM(mmc)));
	  return v;
	}
	return NULL;
}
...
{
  modelica_boolean retVal = (MMC_GETHDR(obj) == MMC_CONSHDR);
  return retVal;
}
...
modelica_metatype valueLst_to_type_descs(modelica_metatype value)
{
  ...
  //Compatability with the existing read and write functions.
