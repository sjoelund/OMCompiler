...
algorithm
  ...
  List.map1_0(functionArgs,genAllocaLLVMInstForVar,64);
  ...
  List.map1_0(functionToBeCalled.outputs,genAllocaLLVMInstForVar,64);
  ...
  res := match mmcVars
    ...
    case {} /*Void functions*/
      algorithm
        ...
        EXT_LLVM.finnishGen();
        runJit(Absyn.pathString(functionToBeCalled.name),valLst);
    ...
    case H :: {} //Functions returning a single value.
      algorithm
        ...
        EXT_LLVM.finnishGen();
      then runJit(Absyn.pathString(functionToBeCalled.name),valLst);
...
algorithm
  ...
  List.map1_0(func.outputs,genAllocaLLVMInstForVar,64);
...
algorithm
  List.map1_0(locals,genAllocaLLVMInstForVar,64);
...
function genAllocaLLVMInstForVar
  ...
  input MidCode.Var var;
  input Integer bitWidth = 64 "Default bitwidth for integers";
protected
  DAE.Type ty;
...
//print("LLVM INST:" + anyString(dest.ty) + "&:" + anyString(rValue) + "\n");
  ...
  case (MidCode.VAR(_,DAE.T_INTEGER(__),_),MidCode.LITERALINTEGER(value=pInteger))
    algorithm
      EXT_LLVM.genStoreLiteralInt(pInteger,genVarName(dest),64 /*Number of bits*/);
    then();
