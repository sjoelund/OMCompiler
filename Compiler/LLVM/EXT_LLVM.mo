...
function setOptSettings
  input Boolean b;
  external "C" setOptSettings(b) annotation(Library = "omcruntime");
end setOptSettings;
...
function genStoreLiteralInt
  ...
  input String dest;
  input Integer nBits;
  external "C" storeLiteralInt(src,dest,nBits) annotation(Library = "omcruntime", Include = "int storeLiteralInt(int64_t src, const char *dest,const uint8_t nBits);");
...
function funcIsJitCompiled
  input String s;
  output Boolean b;
  external "C" b = fIsJitCompiled(s) annotation(Library="omcruntime");
